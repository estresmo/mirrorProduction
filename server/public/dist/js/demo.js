...
(function ($) {
  ...
  function createSkinBlock(colors, callback, noneSelected) {
    ...
    colors.forEach(function (color) {
      var $color = $('<option />', {
        "class": ((0, _typeof2["default"])(color) === 'object' ? color.join(' ') : color).replace('navbar-', 'bg-').replace('accent-', 'bg-'),
        text: capitalizeFirstLetter(((0, _typeof2["default"])(color) === 'object' ? color.join(' ') : color).replace(/navbar-|accent-|bg-/, '').replace('-', ' '))
      });
      $block.append($color);
